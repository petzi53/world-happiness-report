...
class_scheme <- function(df, sel1, sel2) {
  ...
  df |> 
        ...
        dplyr::mutate(N = lengths(gregexpr(";", data)) + 1) |> 
...
In a first step I want to findout  where these classifications systems come from and compare
...
2. Download classification files and store them for faster access as R objects with `r glossary("rds")` format (see @sec-80-download-class-files).
3. Inspect the data classification files of World Bank (@sec-80-inspect-wb) and of the United Nations (@sec-80-unsd) in detail.
::::
:::::
...
### Downloading classification files {#sec-80-download-class-files}

::: {.my-code-collection}
:::: {.my-code-collection-header}
::::: {.my-code-collection-icon}
:::::
:::::: {#exm-download-class-files}
...
###### World Bank {#sec-80-wb}

:::::{.my-r-code}
:::{.my-r-code-header}
:::::: {#cnj-wb-class}
: Download and saving sheets "List of economies" and "compositions" of the World Bank Country Classification File `Class.xlsx`
::::::
:::
::::{.my-r-code-container}
...
downloader::download(
    ...
    destfile = base::paste0(here::here(), "/data/country-class/wb-class.xlsx")
)

wb_class_economies <- readxl::read_xlsx(base::paste0(here::here(), "/data/country-class/wb-class.xlsx"),
                                           sheet = "List of economies") 
pb_save_data_file("country-class", wb_class_economies, "wb_class_economies.rds")

wb_class_compositions <- readxl::read_xlsx(base::paste0(here::here(), "/data/country-class/wb-class.xlsx"),
                                           sheet = "compositions")
pb_save_data_file("country-class", wb_class_compositions, "wb_class_compositions.rds")
...
:::{.my-r-code-header}
:::::: {#cnj-code-name-b}
: Save the manually downloaded UNSD Excel classification file as `.rds` object
::::::
:::
::::{.my-r-code-container}
...
unsd_class <- readxl::read_xlsx(base::paste0(here::here(), 
                 "/data/country-class/unsd-class.xlsx")) 
pb_save_data_file("country-class", unsd_class, "unsd_class.rds")
...
df_wb1 <-  base::readRDS("data/country-class/wb_class_economies.rds") |> 
    dplyr::slice(1:218)



(
    wb1_class <- class_scheme(
            df = df_wb1,
...
`Region` is a coarse classification scheme with only **`r length(wb1_class$x$data$N)` regions formed by `r sum(wb1_class$x$data$N)` countries**.
...
df_wb2 <-  base::readRDS("data/country-class/wb_class_compositions.rds")
...
page](https://unstats.un.org/unsd/methodology/m49/overview/). There is
no URL for an R script available, because triggering one of the buttons
copies or downloads the data with the help of Javascript. So I had to download the file manually. I stored it as `unsd-class.csv` under the "data/country-class" folder. 

The file has 15 columns as you can see from the [Overview
...
destfile <- base::paste0(chap_folder, "unsd_m49.csv")
utils::download.file(url, destfile)
tmp <- readr::read_csv2(destfile)


## save original file without changes
pb_save_data_file("chap80", tmp, "unsd_m49.rds")
```
