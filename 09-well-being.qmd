...
The function `pb_create_folder()` checks if the folder already exists. If this is the case it leaves it untouched, otherwise it creates the folder at the desired path.
...
Tidying the dataset includes to combine country names if the span two lines and to delete the empty lines with the many NA's. Note also that there is a data entry error in the original PDF (see: page 5, second last line, column 6 from the right) that has to be corrected.
...
create_ladder_table <- function(year) {
